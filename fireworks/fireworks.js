...
Fireworks.prototype.createRandom = function (x, y, options) {
	...
	/* random values */
	if (o.shape == -1) {
		o.shape = this._randomShape();
	}
	if (o.color == -1) {
		o.color = (Math.random() > 0.2 ? this._randomColor() : 0);
	}
	if (o.speed == -1) {
		o.speed = 0.5 + Math.random() * 3;
	}
	if (o.particles == -1) {
		o.particles = 50 + Math.round(Math.random() * 200);
	}
	if (o.secondary == -1) {
		o.secondary = Math.random() > 0.6;
	}
...
Fireworks.prototype._step = function () {
	...
	diff = t2 - time || 1;
	var fps = Math.round(60 / diff) + ''
	var count = this._particles.length;
	if (count) {
		this._status.innerHTML = "烟花粒子数量: " + this._particles.length + "<br>FPS:" + (fps.length < 2 ? fps + '0' : fps);
	} else {
...
Fireworks.prototype._redraw = function () {
	...
	while (remove.length) {
		this._removeParticle(remove.pop());
	}
}
